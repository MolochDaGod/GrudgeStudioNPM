import { LOCO_BAKED, bakedAnimUrl, getAnimPack, normalizeAnimPackId } from '@grudge-studio/assets';
...
function packSkillUrls(packId, skillId, sameOrigin = true) {
  ...
  if (!skill) return [];
  return skill.candidates.map((c) => c.key).filter((k) => !k.endsWith(".fbx") && !k.startsWith("_")).map((rel) => bakedAnimUrl(rel, sameOrigin));
}
